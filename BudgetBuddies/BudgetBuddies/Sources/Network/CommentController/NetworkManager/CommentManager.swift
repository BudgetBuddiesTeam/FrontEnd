...
final class CommentManager {
    ...
    // 댓글 하나 가져오기
    typealias GetOneDiscountsCommentsNetworkCompletion = (Result<GetOneDiscountsCommentsResponseDTO, Error>) -> Void
    typealias GetOneSupportsCommentsNetworkCompletion = (Result<GetOneSupportsCommentsResponseDTO, Error>) -> Void
    ...
    func getOneDiscountsComments(commentId: Int, completion: @escaping(GetOneDiscountsCommentsNetworkCompletion)) {
        CommentProvider.request(.getOneDiscountsComments(commentId: commentId)) { result in
            ...
            case .success(let response):
                ...
                do {
                    let decoder = JSONDecoder()
                    let comment = try decoder.decode(GetOneDiscountsCommentsResponseDTO.self, from: response.data)
    ...
    func getOneSupportsComments(commentId: Int, completion: @escaping(GetOneSupportsCommentsNetworkCompletion)) {
        CommentProvider.request(.getOneSupportsComments(commentId: commentId)) { result in
            switch result {
            case .success(let response):
                do {
                    let decoder = JSONDecoder()
                    let comment = try decoder.decode(GetOneSupportsCommentsResponseDTO.self, from: response.data)
                    completion(.success(comment))
