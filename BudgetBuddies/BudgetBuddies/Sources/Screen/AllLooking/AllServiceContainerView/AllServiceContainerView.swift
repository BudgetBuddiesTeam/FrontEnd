...
class AllServiceContainerView: UIView {
  ...
  private static let chevronHeight = 21
  // MARK: - UI Components
  ...
  private let pocketCalendarIcon : UIImageView = {
    ...
    imageView.image = BudgetBuddiesAsset.AppImage.IconForAllLookingView.AllServiceContainer.todayCalendar.image
    return imageView
  }()
  ...
  private let supportInfoConfirmIcon : UIImageView = {
    let imageView = UIImageView()
    imageView.image = BudgetBuddiesAsset.AppImage.IconForAllLookingView.AllServiceContainer.questionMarkCircle.image
    return imageView
  }()
  ...
  private func setLayout() {
    ...
    // 컨테이너 레이아웃
    addSubviews(allServiceText,pocketCalendarContainer, priceEventInfoContainer, supportInfoConfirmContainer)
    ...
    // "주머니 캘린더" 항목들 레이아웃
    pocketCalendarContainer.addSubviews(pocketCalendarIcon, pocketCalendarText, pocketCalendarRightChevron)
    
    pocketCalendarIcon.snp.makeConstraints { make in
      make.centerY.equalToSuperview()
      make.leading.equalToSuperview().inset(leadingDistanceFromContainer)
    }
    ...
    pocketCalendarRightChevron.snp.makeConstraints { make in
      make.centerY.equalToSuperview()
      make.trailing.equalToSuperview().inset(trailingDistanceFromContainer)
    }
    
    // "이번 달 할인정보 확인하기" 항목들 레이아웃
    priceEventInfoContainer.addSubviews(priceEventInfoConfirmIcon, priceEventInfoConfirmText, priceEventInfoConfirmRightChevron)
    ...
    supportInfoConfirmContainer.addSubviews(supportInfoConfirmIcon, supportInfoConfirmText, supportInfoConfirmRightChevron)
