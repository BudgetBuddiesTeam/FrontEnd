...
final class MonthReportViewController: UIViewController {
    ...
    lazy var accountBookView = {
        ...
        view.totalButton.addTarget(self, action: #selector(accountBookButtonTapped), for: .touchUpInside)
        return view
    }()
...
extension MonthReportViewController {
    ...
    func loadTopGoal() {
        services.consumeGoalService.getTopGoal(userId: 1, peerAgeStart: 23, peerAgeEnd: 25, peerGender: "male") { result in
            switch result {
            case .success(let response):
    ...
    func loadTopConsumption() {
        services.consumeGoalService.getTopConsumption(userId: 1, peerAgeStart: 22, peerAgeEnd: 25, peerGender: "male") { result in
            switch result {
            case .success(let response):
    ...
    func loadTopConsumptions() {
        services.consumeGoalService.getTopConsumptions(userId: 1, peerAgeStart: 22, peerAgeEnd: 25, peerGender: "male") { result in
            switch result {
            case .success(let response):
