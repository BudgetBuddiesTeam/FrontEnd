...
class ConsumeView: UIView {
  ...
  public var consumedPriceTextField: UITextField = {
    ...
    textField.layer.backgroundColor = BudgetBuddiesAppAsset.AppColor.textBox.color.cgColor
    ...
    textField.font = BudgetBuddiesAppFontFamily.Pretendard.regular.font(size: 16)  // 폰트 설정
    return textField
  }()
  ...
  public var categorySettingButton: UIButton = {
    ...
    button.titleLabel?.font = BudgetBuddiesAppFontFamily.Pretendard.medium.font(size: 16)

    // 버튼 이미지 "chevron" 설정 코드
    button.setImage(UIImage(systemName: "chevron.right"), for: .normal)
    // chevron의 컬러를 주기 위해서는 title과 달리 메소드를 사용하지 않고 tintColor라는 프로퍼티에 직접 접근합니다.
    button.tintColor = BudgetBuddiesAppAsset.AppColor.subGray.color
