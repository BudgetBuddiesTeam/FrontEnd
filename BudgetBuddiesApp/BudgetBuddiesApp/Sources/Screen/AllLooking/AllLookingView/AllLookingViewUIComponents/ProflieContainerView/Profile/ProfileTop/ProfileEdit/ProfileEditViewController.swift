...
class ProfileEditViewController: UIViewController {
    ...
    private func setupButtonActions() {
        // 성별 버튼
        profileEditView.maleButton.addTarget(self, action: #selector(didTapGenderButton), for: .touchUpInside)
        profileEditView.femaleButton.addTarget(self, action: #selector(didTapGenderButton), for: .touchUpInside)
        
        // 나이 버튼
        profileEditView.ageButtonArray.forEach { $0.addTarget(self, action: #selector(didTapAgeRadioButton), for: .touchUpInside) }
        
        // 계속하기 버튼
        profileEditView.keepGoingButton.addTarget(self, action: #selector(didTapKeepGoingButton), for: .touchUpInside)
    }
