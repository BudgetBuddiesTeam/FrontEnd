...
class ProfileViewController: UIViewController {
  ...
  private func setTapGesture() {
    ...
    /// 사용자 설정의 계정 정보 탭
    let accountInfoViewTapped = UITapGestureRecognizer(
      target: self, action: #selector(accountInfoViewTapped))
    profileView.settingContainerView.accountInfoContainer.addGestureRecognizer(
      accountInfoViewTapped)

    /// 사용자 설정의 추가 정보 탭
    let addtionInfoViewTapped = UITapGestureRecognizer(
      target: self, action: #selector(addtionInfoViewTapped))
    profileView.settingContainerView.addtionInfoContainer.addGestureRecognizer(
      addtionInfoViewTapped)

    /// 사용자 설정의 알림 설정 탭
    let notificationViewTapped = UITapGestureRecognizer(
      target: self, action: #selector(notificationViewTapped))
    profileView.settingContainerView.notificationContainer.addGestureRecognizer(
      notificationViewTapped)
