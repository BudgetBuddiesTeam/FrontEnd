...
class SubLabel: UILabel {
  ...
  private func setLabel() {
    ...
    attributedString.addAttribute(
      .font, value: BudgetBuddiesAppFontFamily.Pretendard.regular.font(size: 14), range: fullRange)
    attributedString.addAttribute(
      .foregroundColor, value: BudgetBuddiesAppAsset.AppColor.subGray.color, range: fullRange)
    ...
    if let loginRange = fullText.range(of: yellowText) {
      ...
      attributedString.addAttribute(
        .foregroundColor, value: BudgetBuddiesAppAsset.AppColor.coreYellow.color, range: nsRange)  // 노란색 적용
      if isLined {
        attributedString.addAttribute(
          .underlineStyle, value: NSUnderlineStyle.single.rawValue, range: nsRange)  // 밑줄 적용
      }
    }
