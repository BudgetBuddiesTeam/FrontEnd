...
class AdditionalInformationView: UIView {
  ...
  lazy var mobileCarrierStackView: UIStackView = {
    let sv = UIStackView(arrangedSubviews: [
      mobileCarrierLabel, firstMCStackView, secondMCStackView, thirdMCStackView,
    ])
    sv.axis = .vertical
    sv.spacing = 8
    sv.alignment = .fill
  ...
  let entertainmentExpensesButton = ClearBackgroundCheckBoxButton(
    interestCategory: .entertainmentExpenses)
  ...
  let transportationExpensesButton = ClearBackgroundCheckBoxButton(
    interestCategory: .transportationExpenses)
  let familyExpensesButton = ClearBackgroundCheckBoxButton(interestCategory: .familyEventExpenses)
  let regularPaymentExpensesButton = ClearBackgroundCheckBoxButton(
    interestCategory: .regularPaymentExpenses)
  ...
  lazy var interestedCategoryStackView: UIStackView = {
    let sv = UIStackView(arrangedSubviews: [
      interestCategorylabel, firstICStackView, secondICStackView,
    ])
    sv.axis = .vertical
    sv.spacing = 12
    sv.alignment = .fill
  ...
  private func addButtonsToArray() {
    self.mobileCarrierButtonArray.append(contentsOf: [
      sktButton, ktButton, lgUPlusButton, thriftyPhoneButton, elseButton,
    ])

    self.interestCategoryButtonArray.append(contentsOf: [
      foodExpensesButton, entertainmentExpensesButton, cafeExpensesButton, shoppingExpensesButton,
      fashionExpensesButton, cultureExpensesButton, transportationExpensesButton,
      familyExpensesButton, regularPaymentExpensesButton,
    ])
  }
  ...
  func scrollToBottom(animated: Bool) {
    let bottomOffset = CGPoint(
      x: 0, y: scrollView.contentSize.height - scrollView.bounds.size.height)
    if bottomOffset.y > 0 {
  ...
  private func setupUI() {
    ...
    contentView.addSubviews(
      stepDot, titleStackView, regionStackView, mobileCarrierStackView, interestedCategoryStackView)
