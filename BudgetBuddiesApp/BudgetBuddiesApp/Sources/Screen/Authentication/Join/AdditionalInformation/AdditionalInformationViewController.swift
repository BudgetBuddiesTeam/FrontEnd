...
class AdditionalInformationViewController: UIViewController {
  ...
  var isRegionPicked: Bool = false {
    didSet {
      additionalInformationView.continueButtonToggle(
        isRegionPicked, isMobileCarrierSelected, isInterestCategoriesSelected)
    }
  }
  ...
  private func setupButtonActions() {
    // 거주지역 선택 tapGesture
    additionalInformationView.regionPickerView.addGestureRecognizer(
      UITapGestureRecognizer(target: self, action: #selector(didTapRegionPicker)))
    ...
    // 건너뛰기 버튼
    additionalInformationView.skipButton.addTarget(
      self, action: #selector(didTapSkipButton), for: .touchUpInside)

    // 선택 후 계속하기 버튼
    additionalInformationView.continueButton.addTarget(
      self, action: #selector(didTapContinueButton), for: .touchUpInside)
  }
