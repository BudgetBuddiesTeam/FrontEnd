...
class BasicInformationViewController: UIViewController {
  ...
  private func setupButtonActions() {
    // 성별 버튼
    basicInformationView.maleButton.addTarget(
      self, action: #selector(didTapGenderButton), for: .touchUpInside)
    basicInformationView.femaleButton.addTarget(
      self, action: #selector(didTapGenderButton), for: .touchUpInside)

    // 나이 버튼
    basicInformationView.ageButtonArray.forEach {
      $0.addTarget(self, action: #selector(didTapAgeRadioButton), for: .touchUpInside)
    }

    // 계속하기 버튼
    basicInformationView.keepGoingButton.addTarget(
