...
class RegionPickerViewController: DimmedViewController {
  ...
  private func registerCells() {
    regionPicker.regionTableView.register(
      RegionCell.self, forCellReuseIdentifier: RegionCell.identifier)
  }
  ...
  private func setupRegionPicker() {
    ...
    regionPicker.snp.makeConstraints { make in
      ...
      regionPickerTopConstraint =
        make.top.equalTo(self.view.safeAreaLayoutGuide.snp.top).inset(regionPickerValue).constraint
...
extension RegionPickerViewController: UITableViewDataSource {
  ...
  func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
    let regionCell =
      regionPicker.regionTableView.dequeueReusableCell(
        withIdentifier: RegionCell.identifier, for: indexPath) as! RegionCell
