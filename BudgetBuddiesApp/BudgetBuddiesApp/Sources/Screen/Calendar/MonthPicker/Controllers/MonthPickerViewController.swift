    ...
    -> UICollectionViewCell
  {
    ...
    if let month = yearMonth?.month {
      if indexPath.row + 1 == month {
        monthCell.backView.backgroundColor = BudgetBuddiesAppAsset.AppColor.coreYellow.color
        monthCell.monthLabel.textColor = BudgetBuddiesAppAsset.AppColor.white.color
      } else {
        monthCell.backView.backgroundColor = .clear
        monthCell.monthLabel.textColor = BudgetBuddiesAppAsset.AppColor.subGray.color
      }
    }
...
extension MonthPickerViewController: UICollectionViewDelegate {
  func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
    ...
    let selectedColor = BudgetBuddiesAppAsset.AppColor.coreYellow.color
    let unSelectedLabelColor = BudgetBuddiesAppAsset.AppColor.subGray.color
